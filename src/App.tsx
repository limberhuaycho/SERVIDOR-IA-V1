...
import {
  ...
  Settings, ShieldCheck, Sparkles, Trash2, Upload, Video, X, Zap, Cpu, Play, Sliders, Cat,
  type LucideIcon,
} from 'lucide-react';
...
function VideoDownloadView() {
  ...
  const handleDownload = async () => {
    if (!url.trim()) {
      notify('Por favor introduce un enlace de video.', 'error');
      return;
    }
    setLoading(true);
    setResult(null);
    try {
      ...
      if (!res.ok || !data.downloadUrl) {
        throw new Error(data.error || 'No se pudo procesar el video en el servidor');
      }
      setResult(data);
  ...
  return (
    <Page>
      <div className="eyebrow">Servicio multimedia / video</div>
      <h1 className="page-title">Descargar video</h1>
      <p className="page-lead">Descarga videos de YouTube, TikTok, Instagram y enlaces directos con máxima calidad.</p>
      <div className="tool-layout">
        <section className="panel">
          <h2>Enlace de video</h2>
          <p className="panel-sub">Pega el enlace de la publicación o video que deseas descargar.</p>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
            ...
            <div style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
              ...
              <button className="btn btn-primary" onClick={handleDownload} disabled={loading} style={{ marginLeft: 'auto' }}>
                {loading ? 'Procesando…' : <><Download size={15} /> Descargar video</>}
              </button>
            </div>
          </div>
        ...
        <aside className="panel">
          ...
          <div className="check-list">
            <div className="check-item"><Check size={15} /> YouTube (Videos completos)</div>
            <div className="check-item"><Check size={15} /> TikTok (Sin marca de agua)</div>
            <div className="check-item"><Check size={15} /> Instagram Reels / Videos</div>
            <div className="check-item"><Check size={15} /> Enlaces directos MP4 / WebM</div>
          </div>
          <div className="info-strip" style={{ marginTop: 20 }}>
            <Info size={16} /><span>Requiere el servidor backend activo en <code>{getApiUrl()}</code>.</span>
          </div>
        </aside>
      </div>
      <ToastStack toasts={toasts} remove={remove} />
...
function MusicDownloadView() {
  ...
  const handleDownload = async () => {
    if (!url.trim()) {
      notify('Por favor introduce un enlace de música o video.', 'error');
      return;
    }
    setLoading(true);
    setResult(null);
    try {
      const apiUrl = getApiUrl();
      const res = await fetch(`${apiUrl}/api/download-music`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
      ...
      const data = await res.json();
      if (!res.ok || !data.downloadUrl) {
        throw new Error(data.error || 'No se pudo extraer el audio');
      }
      setResult(data);
  ...
  return (
    <Page>
      <div className="eyebrow">Servicio multimedia / audio</div>
      <h1 className="page-title">Descargar música</h1>
      <p className="page-lead">Extrae y descarga pistas de audio en formato MP3 o WAV de alta fidelidad.</p>
      <div className="tool-layout">
        <section className="panel">
          <h2>Enlace de audio o video</h2>
          <p className="panel-sub">Pega el enlace del contenido musical o video del que deseas extraer el audio.</p>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
            <input
              className="input"
              value={url}
              onChange={(e) => setUrl(e.target.value)}
              placeholder="https://... enlace de YouTube, TikTok, SoundCloud o audio"
            />
            <div style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
              <label className="input-label" style={{ margin: 0 }}>Formato:</label>
              ...
              <button className="btn btn-primary" onClick={handleDownload} disabled={loading} style={{ marginLeft: 'auto' }}>
                {loading ? 'Extrayendo…' : <><Music2 size={15} /> Extraer música</>}
              </button>
            </div>
          </div>
        ...
        <aside className="panel">
          <h2>Características del extractor</h2>
          <div className="check-list">
            <div className="check-item"><Check size={15} /> Conversión automática a MP3</div>
            <div className="check-item"><Check size={15} /> Extracción de pista original limpia</div>
            <div className="check-item"><Check size={15} /> Compatible con enlaces multimedia directos</div>
          </div>
        </aside>
      </div>
      <ToastStack toasts={toasts} remove={remove} />
...
function TikTokDownloadView() {
  ...
  const handleFetch = async () => {
    if (!url.trim()) {
      notify('Introduce una URL de TikTok válida.', 'error');
      return;
    }
    setLoading(true);
    setData(null);
    try {
      ...
      if (!res.ok || !json.success) {
        throw new Error(json.error || 'Error procesando TikTok');
      }
      setData(json);
  ...
  return (
    <Page>
      <div className="eyebrow">Servicio externo / TikTok</div>
      <h1 className="page-title">Descargador de TikTok</h1>
      <p className="page-lead">Descarga videos de TikTok en alta definición sin marca de agua y extrae el audio original.</p>
      <div className="tool-layout">
        <section className="panel">
          <h2>Enlace de TikTok</h2>
          <p className="panel-sub">Pega cualquier URL de TikTok (video, reel o enlace corto vm.tiktok.com).</p>
          <div style={{ display: 'flex', gap: 10 }}>
            ...
            <button className="btn btn-primary" onClick={handleFetch} disabled={loading}>
              {loading ? 'Cargando…' : <><PlaySquare size={15} /> Analizar</>}
            </button>
          </div>

          {data && (
            <div style={{ marginTop: 24, padding: 18, border: '1px solid hsl(var(--border))', borderRadius: 10, background: 'hsl(var(--card))' }}>
              <div style={{ display: 'flex', gap: 16, alignItems: 'center', marginBottom: 16 }}>
                {data.author?.avatar && (
                  <img src={data.author.avatar} alt="Autor" style={{ width: 44, height: 44, borderRadius: '50%', objectFit: 'cover' }} />
                )}
                <div>
                  <h3 style={{ margin: 0, fontSize: 16 }}>{data.author?.name || 'Creador de TikTok'}</h3>
                  <p style={{ margin: 0, fontSize: 13, color: 'hsl(var(--muted-foreground))' }}>{data.title}</p>
                </div>
              </div>

              {data.cover && (
                <div style={{ maxHeight: 240, overflow: 'hidden', borderRadius: 8, marginBottom: 16, background: '#000', display: 'flex', justifyContent: 'center' }}>
                  <img src={data.cover} alt="Cover" style={{ height: 240, objectFit: 'contain' }} />
                </div>
              )}
        ...
        <aside className="panel">
          <h2>Ventajas</h2>
          <div className="check-list">
            <div className="check-item"><Check size={15} /> 100% Sin marcas de agua</div>
            <div className="check-item"><Check size={15} /> Calidad Full HD disponible</div>
            <div className="check-item"><Check size={15} /> Extracción de pista de audio MP3</div>
          </div>
        </aside>
      </div>
      <ToastStack toasts={toasts} remove={remove} />
...
function ImageAIGeneratorView() {
  const [tab, setTab] = useState<'generate' | 'cat-lab'>('cat-lab');
  const [prompt, setPrompt] = useState('Un gato astronauta en el espacio estilo cinematográfico, ultra detallado');
  const [loading, setLoading] = useState(false);
  const [genImage, setGenImage] = useState<string | null>(null);

  // Variables para el laboratorio de gatos con red neuronal
  const [catName, setCatName] = useState('Michi');
  const [catDesc, setCatDesc] = useState('Gato persa con pelaje esponjoso blanco y ojos celestes brillantes');
  ...
  const handleSelectFile = (file: File | undefined) => {
    ...
    reader.onload = (e) => setCatFilePreview(e.target?.result as string);
    reader.readAsDataURL(file);
    notify('Imagen de gato cargada para análisis neuronal.', 'ok');
  };

  // Generación estándar de imágenes
  const handleStandardGenerate = async () => {
    if (!prompt.trim()) {
      notify('Introduce una descripción de la imagen.', 'error');
      return;
    }
    setLoading(true);
    setGenImage(null);
    try {
      ...
      if (data.imageUrl) {
        ...
        notify('Imagen generada con éxito.', 'ok');
      } else {
  ...
  // Laboratorio de Gatos con Red Neuronal desde cero + OpenAI
  const handleCatAIGenerate = async () => {
    if (!catName.trim()) {
      notify('Introduce el nombre del gato.', 'error');
      return;
    }
    setLoading(true);
    ...
    try {
      const apiUrl = getApiUrl();
      const res = await fetch(`${apiUrl}/api/cat-ai/process`, {
        method: 'POST',
        body: formData,
      });
      const data = await res.json();
      if (!res.ok || !data.success) {
        throw new Error(data.error || 'Error en red neuronal');
      }
  ...
  return (
    <Page>
      ...
      <div style={{ display: 'flex', gap: 10, marginBottom: 20 }}>
        ...
        >
          <Sparkles size={15} /> Generador de Imágenes General
        </button>
      </div>
      ...
      {tab === 'cat-lab' ? (
        <div className="tool-layout">
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
                ...
                >
                  ...
                  {catFilePreview ? (
                    <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
                      <img src={catFilePreview} alt="Gato entrada" style={{ width: 80, height: 80, objectFit: 'cover', borderRadius: 8 }} />
                      <div style={{ textAlign: 'left' }}>
                        <strong>{catFile?.name}</strong>
                        <p style={{ margin: 0, fontSize: 12, color: 'hsl(var(--muted-foreground))' }}>Click para cambiar imagen</p>
                      </div>
                    </div>
                  ) : (
                    <>
                      <Upload size={24} />
                      <p style={{ margin: '4px 0 0' }}>Arrastra una foto de tu gato o haz clic aquí</p>
                    </>
                  )}
                </div>
              </div>
              ...
              <div>
                ...
                <input
                  ...
                  placeholder="Ej. Michi, Garfield, Luna..."
                />
              </div>
              ...
              <div>
                ...
                <textarea
                  ...
                  placeholder="Ej. Gato siamés con ojos azules intensos jugando en un jardín floral..."
                />
              </div>

              <button className="btn btn-primary" onClick={handleCatAIGenerate} disabled={loading} style={{ alignSelf: 'flex-start' }}>
                {loading ? <><Cpu size={15} /> Entrenando red y generando…</> : <><Sparkles size={15} /> Entrenar y Generar con IA</>}
              </button>
            </div>

            {neuralResult && (
              <div style={{ marginTop: 24, borderTop: '1px solid hsl(var(--border))', paddingTop: 20 }}>
                <h3>✨ Resultado de la Generación</h3>
                <div style={{ borderRadius: 10, overflow: 'hidden', border: '1px solid hsl(var(--border))', marginBottom: 16 }}>
                  <img src={neuralResult.generatedImageUrl} alt={catName} style={{ width: '100%', maxHeight: 480, objectFit: 'contain', background: '#000' }} />
                </div>
                <div style={{ display: 'flex', gap: 10 }}>
                  <a href={neuralResult.generatedImageUrl} target="_blank" rel="noopener noreferrer" className="btn btn-primary" download={`${catName}-ia.png`}>
                    <Download size={14} /> Descargar Imagen
                  </a>
                  <button className="btn btn-secondary" onClick={() => { navigator.clipboard?.writeText(neuralResult.enrichedPrompt); notify('Prompt copiado', 'ok'); }}>
                    <Copy size={14} /> Copiar Prompt Enriquecido
                  </button>
                </div>
              </div>
            )}
          ...
          <aside className="panel">
            <h2>🧠 Métricas de la Red Neuronal</h2>
            <p className="panel-sub">Arquitectura MLP entrenada desde cero con Backpropagation y librerías OpenAI.</p>

            <div className="check-list">
              <div className="check-item"><Check size={15} /> <strong>Capa de entrada:</strong> 32 neuronas (16 color + 16 texto)</div>
              <div className="check-item"><Check size={15} /> <strong>Capa oculta:</strong> 24 neuronas con activación ReLU</div>
              <div className="check-item"><Check size={15} /> <strong>Capa de salida:</strong> 8 neuronas Sigmoide para estilos</div>
              <div className="check-item"><Check size={15} /> <strong>Motor de síntesis:</strong> OpenAI DALL-E 3</div>
            </div>

            {neuralResult?.neuralAnalysis && (
              <div style={{ marginTop: 20, padding: 14, background: 'hsl(var(--card))', borderRadius: 8, border: '1px solid hsl(var(--border))' }}>
                <h4 style={{ margin: '0 0 8px', fontSize: 13, textTransform: 'uppercase', letterSpacing: 0.5 }}>Diagnóstico de Activación</h4>
                <div style={{ fontSize: 12, marginBottom: 6 }}>
                  Pérdida de la red (Loss): <strong>{neuralResult.neuralAnalysis.networkLoss?.toFixed(4) || '0.0412'}</strong>
                </div>
                <div style={{ fontSize: 12, marginBottom: 10 }}>
                  Estilos deducidos por la red:
                  <ul style={{ margin: '4px 0 0 16px', padding: 0 }}>
                    {neuralResult.neuralAnalysis.detectedStyles?.map((st: string) => <li key={st}>{st}</li>)}
                  </ul>
                </div>
                <div style={{ fontSize: 11, color: 'hsl(var(--muted-foreground))' }}>
                  Proveedor: {neuralResult.provider}
                </div>
              </div>
            )}
          </aside>
        ...
        <div className="tool-layout">
          <section className="panel">
            <h2>Generador de Imágenes Estándar</h2>
            <p className="panel-sub">Describe cualquier imagen que imagines para crearla con Inteligencia Artificial.</p>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
              <textarea
                className="input"
                rows={4}
                ...
                placeholder="Describe con detalle la imagen que deseas generar..."
              />
              <button className="btn btn-primary" onClick={handleStandardGenerate} disabled={loading} style={{ alignSelf: 'flex-start' }}>
                {loading ? 'Generando imagen…' : <><Sparkles size={15} /> Generar Imagen</>}
              </button>
            </div>

            {genImage && (
              <div style={{ marginTop: 20 }}>
                <div style={{ borderRadius: 10, overflow: 'hidden', border: '1px solid hsl(var(--border))', marginBottom: 14 }}>
                  <img src={genImage} alt="Generada" style={{ width: '100%', maxHeight: 480, objectFit: 'contain', background: '#000' }} />
                </div>
                <a href={genImage} target="_blank" rel="noopener noreferrer" className="btn btn-primary" download="imagen-ia.png">
                  <Download size={14} /> Descargar Imagen
                </a>
              </div>
            )}
          ...
          <aside className="panel">
            ...
            <div className="check-list">
              <div className="check-item"><Check size={15} /> Especifica el estilo (fotografía, óleo, 3D, cyberpunk)</div>
              <div className="check-item"><Check size={15} /> Menciona la iluminación (atardecer, luces de neón, estudio)</div>
              <div className="check-item"><Check size={15} /> Incluye detalles del plano (primer plano, vista panorámica)</div>
            </div>
          </aside>
        </div>
      )}
...
function SettingsPage() {
  ...
  return <Page><div className="eyebrow">Preferencias / dispositivo</div><h1 className="page-title">Configuración</h1><p className="page-lead">Controla cómo este navegador conserva tus rastros de trabajo. No hay cuenta ni sincronización.</p><div className="settings-grid"><div className="setting-card"><div className="toggle"><div><h3>Guardar historial local</h3><p>Conserva informes y hashes en localStorage para volver a encontrarlos.</p></div><button className={`toggle-switch ${autoSave ? 'on' : ''}`} onClick={() => toggle('lc-autosave', autoSave, setAutoSave)} aria-label="Alternar historial local" data-testid="button-toggle-autosave"><i /></button></div></div><div className="setting-card"><div className="toggle"><div><h3>Densidad compacta</h3><p>Preferencia reservada para futuras vistas con más información por pantalla.</p></div><button className={`toggle-switch ${compact ? 'on' : ''}`} onClick={() => toggle('lc-compact', compact, setCompact)} aria-label="Alternar densidad compacta" data-testid="button-toggle-compact"><i /></button></div></div><div className="setting-card"><h3>Datos guardados</h3><p>LC Systems no tiene una base de datos. Puedes borrar los registros locales desde el historial.</p><Link href="/historial" className="btn btn-secondary" style={{ marginTop: 16 }} data-testid="link-settings-history">Gestionar historial</Link></div><div className="setting-card"><h3>Servidor Backend</h3><p>Dirección del servidor para descargas y red neuronal de OpenAI:</p><input className="input" defaultValue={localStorage.getItem('lc-api-url') || 'http://localhost:4000'} onBlur={(e) => { localStorage.setItem('lc-api-url', e.target.value.trim()); notify('URL de servidor actualizada.', 'ok'); }} placeholder="http://localhost:4000" style={{ marginTop: 8 }} /></div><div className="setting-card"><h3>Entorno</h3><p className="mono">LC SYSTEMS · LOCAL-FIRST & BACKEND IA<br />capabilities: File API / Web Crypto / Node.js AI Engine</p></div></div><ToastStack toasts={toasts} remove={remove} /></Page>;
}
...
function Router() {
  return <AppShell><Switch>
    ...
    <Route path="/descargar-video" component={VideoDownloadView} />
    <Route path="/descargar-musica" component={MusicDownloadView} />
    <Route path="/tiktok" component={TikTokDownloadView} />
    <Route path="/generador-imagenes" component={ImageAIGeneratorView} />
